...
# from .apimodels import Error, PROTerm, Annotation, Evidence, PAF
...
PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
PREFIX oboInOwl: <http://www.geneontology.org/formats/oboInOwl#> 
SELECT distinct ?ancestor as ?PRO ?directParent
where {
  {
        {
      ...
      WHERE 
      {
          values ?PRO_term {obo:PR_000000468}
         ?PRO_term rdfs:subClassOf ?parent .
...
union {
 {
      ...
      WHERE 
      {
          values ?PRO_term {obo:PR_000000468}
         ?child rdfs:subClassOf  ?PRO_term .
          ?ancestor rdfs:subClassOf* ?child.
          ?ancestor oboInOwl:id ?ancestor_id .
          FILTER(STRSTARTS(?ancestor_id, 'PR:')|| STRSTARTS(?ancestor_id, 'GO:'))
          FILTER (!isBlank(?ancestor)) 
      }
        ...
        union {
                values ?ancestor {obo:PR_000037190}
...
class SparqlSearch:
    ...
    def oboSearch(self, searchParamter):
        proID = searchParamter.searchValue;
        obo = dict();
    ...
    def createOBOStanza(self, data):
        ...
        if 'synomym' in data.keys() and data['synonym']:
    ...
    def pafSearch(self, searchParameter):
        ...
        where = values + self.conf.pafFileQuery
        query = self.conf.cfg['prefix'] + "\n"
        query += "SELECT\n"
        query += select + "\n"
        query += "WHERE\n"
...
# zida
    ...
    def TreeSearch(self, searchParameter):
        proIDsQuery = self.conf.where["TREE"]
        proIDs, error = self.executeQuery(proIDsQuery)
        proTerms = [];
        if error == None:
            for proId in proIDs:
    ...
    def constructFilterQuery(self, searchParameter):
        ...
        if searchParameter.searchField.upper() == "ALLFIELDS" or searchParameter.searchField.upper() == "ANY_FIELD":
            ...
            #query += self.conf.searchField[searchField]
            query += "{\n"
    ...
    def constructEcoCycFilterQuery(self, query, searchParameter, value):
        ...
        if value == "NULL" or value == "":
            ...
            filter = "      FILTER("
            for val in values:
                filter += "STRSTARTS(UCASE(STR(?EcoCycID)), UCASE('HGNC:" + val + "')) ||"
